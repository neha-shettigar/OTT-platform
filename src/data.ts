export const data = [
  {
    id: 1,

    title: 'After Earth',

    poster:
      'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQj4S6mZQ5OFbXn_IkfXt80Zr2KE4P1z0mZ_BraDNNzLOWoheea_EvB9sTGZQaBf92_mNY&usqp=CAU',

    year: 2014,

    category: 'Movie',

    rating: 'UG',

    link: 'https://imdb8.p.rapidapi.com/auto-complete',

    description:
      "A crash landing leaves Kitai Raige and his father Cypher stranded on Earth, a millennium after events forced humanity's escape. With Cypher injured, Kitai must embark on a perilous journey to signal for help.",

    isBookmarked: false,

    isTrending: true,
  },

  {
    id: 2,

    title: 'Avengers',

    poster:
      'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTc4HaVJAPPnby7DdzPKi3QkDh3o9CJgcDFYQ&usqp=CAU',

    year: 2020,

    category: 'Movie',

    rating: 'UG',

    link: '6ZfuNTqbHE8',

    description:
      "Earth's Mightiest Heroes stand as the planet's first line of defense against the most powerful threats in the universe. The Avengers began as a group of extraordinary individuals who were assembled to defeat Loki and his Chitauri army in New York City.",

    isBookmarked: false,

    isTrending: true,
  },

  {
    id: 3,

    title: 'Stranger Things',

    poster:
      'https://mir-s3-cdn-cf.behance.net/project_modules/fs/8a6a68144592045.628efcd3e77b5.jpg',

    year: 2019,

    category: 'TV Series',

    rating: 'A',

    link: 'b9EkMc79ZSU',

    description:
      "When a young boy vanishes, a small town uncovers a mystery involving secret experiments, terrifying supernatural forces and one strange little girl. Watch all you want. This nostalgic nod to '80s sci-fi and horror classics has earned dozens of Emmy nominations, including three for Outstanding Drama.",

    isBookmarked: false,

    isTrending: true,
  },

  {
    id: 4,

    title: 'Lord of the rings',

    poster: 'https://m.media-amazon.com/images/I/91Ju+9pP9kL._SL1500_.jpg',

    year: 2019,

    category: 'Movie',

    rating: 'U',

    link: 'Tl44vcVIRmQ',

    description:
      "The Lord of the Rings is the saga of a group of sometimes reluctant heroes who set forth to save their world from consummate evil. Its many worlds and creatures were drawn from Tolkien's extensive knowledge of philology and folklore.",

    isBookmarked: false,

    isTrending: true,
  },

  {
    id: 5,

    title: 'Star Trek',

    poster:
      'https://i.pinimg.com/originals/e1/8d/d3/e18dd330492c6f0a01ec6901c07a7053.jpg',

    year: 2018,

    category: 'Movie',

    rating: 'UG',

    link: 'QAEkuVgt6Aw',

    description:
      'Star Trek was created by American writer and producer Gene Roddenberry and chronicles the exploits of the crew of the starship USS Enterprise, whose five-year mission is to explore space and, as stated in the title sequence, “to seek out new life and new civilizations, to boldly go where no man has gone before.”',

    isBookmarked: true,

    isTrending: true,
  },

  {
    id: 6,

    title: 'Game of thrones',

    poster:
      'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTV2HPD_77fmLNABeVkMrddirxnh2C7Pm7FJQ&usqp=CAU',

    year: 2019,

    category: 'TV Series',

    rating: 'A',

    link: 'KPLWWIOCOOQ',

    description:
      'In the Game of Thrones, you either win or you die. In the mythical continent of Westeros, nine families of higher nobility (Targaryen, Lannisters, Starks, Tyrell, Martell, Greyjoys, Baratheons and Boltons) scramble bitterly to gain power over the seven kingdoms and the Iron throne.',

    isBookmarked: true,

    isTrending: false,
  },
  {
    id: 7,

    title: 'The Imperfects',

    poster:
      'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSJ2T1EFUMmRGvoO7oWHBdoy4vN6wluTJFZiw&usqp=CAU',

    year: 2022,

    category: 'TV Series',

    rating: 'UG',

    link: 'sTJfJoaZrCw',

    description:
      "From the producers of The Order, Wu Assassins and Van Helsing, The Imperfects is a story of three young adults coming of age while grappling with the burden of being extraordinary. And while it includes plenty of laughs, this series also has some pretty graphic moments, so it's not really one to watch with the kids.",

    isBookmarked: false,

    isTrending: false,
  },

  {
    id: 8,

    title: 'Lost in Space',

    poster: 'https://i.ytimg.com/vi/bg6Dv9Vbfao/maxresdefault.jpg',

    year: 2021,

    category: 'TV Series',

    rating: 'PG',

    link: 'fzmM0AB60QQ',

    description:
      "After crash-landing on an alien planet, the Robinson family fights against all odds to survive and escape. But they're surrounded by hidden dangers.",

    isBookmarked: false,

    isTrending: false,
  },

  {
    id: 9,

    title: 'Pushpa: The rise',

    poster:
      'https://www.iwmbuzz.com/wp-content/uploads/2021/11/viral-alert-allu-arjun-flaunts-his-swag-in-new-poster-of-pushpa-fans-love-it.jpeg',

    year: 2022,

    category: 'Movie',

    rating: 'UG',

    link: 'pKctjlxbFDQ',

    description:
      "'Pushpa: The Rise,' directed by Sukumar and featuring Allu Arjun as the protagonist, is a Telugu language film that narrates the story of a laborer who rises in power and becomes a part of the red sander syndicate. Pushpa is the name of the protagonist, who fears nothing and strives for ultimate power and control.",

    isBookmarked: true,

    isTrending: false,
  },

  {
    id: 10,

    title: 'Lagaan',

    poster:
      'https://media5.newsnationtv.com/photos/2018/06/13/2843_lagaan_7218.jpg',

    year: 2001,

    category: 'Movie',

    rating: 'A',

    link: 'Nhi4Azs2nEw',

    description:
      "Set in 1893, during the late Victorian period of India's colonial British Raj, the film follows the inhabitants of a village in Central India, who, burdened by high taxes and several years of drought, are challenged by an arrogant British Indian Army officer to a game of cricket as a wager to avoid paying the taxes",

    isBookmarked: true,

    isTrending: false,
  },

  {
    id: 11,

    title: 'Army of the dead',

    poster:
      'https://m.media-amazon.com/images/M/MV5BNGY0NzgzYzctYWQwMC00MzM2LThjNGMtZjFjMWUyNzg0ZmM0XkEyXkFqcGdeQXVyODk4OTc3MTY@._V1_.jpg',

    year: 2021,

    category: 'Movie',

    rating: 'UG',

    link: 'tI1JGPhYBS8',

    description:
      'Following a zombie outbreak in Las Vegas, a group of mercenaries take the ultimate gamble, venturing into the quarantine zone to pull off the greatest heist ever attempted.',

    isBookmarked: false,

    isTrending: false,
  },
  {
    id: 12,

    title: 'Umbrella Academy',

    poster: 'https://m.media-amazon.com/images/I/91LnmqN5NLL.jpg',

    year: 2019,

    category: 'TV Series',

    rating: 'PG',

    link: '0DAmWHxeoKw',

    description:
      "Created for Netflix by Steve Blackman and developed by Jeremy Slater, it revolves around a dysfunctional family of adopted sibling superheroes who reunite to solve the mystery of their father's death and the threat of an imminent apocalypse.",

    isBookmarked: false,

    isTrending: true,
  },

  {
    id: 13,

    title: 'Brooklyn Nine-Nine',

    poster:
      'https://m.media-amazon.com/images/M/MV5BNzVkYWY4NzYtMWFlZi00YzkwLThhZDItZjcxYTU4ZTMzMDZmXkEyXkFqcGdeQXVyODUxOTU0OTg@._V1_.jpg',

    year: 2013,

    category: 'TV Series',

    rating: 'UG',

    link: 'faJAT35j5Ss',

    description:
      "Comedy series following the exploits of Det. Jake Peralta and his diverse, lovable colleagues as they police the NYPD's 99th Precinct. Captain Ray Holt takes over Brooklyn's 99th precinct, which includes Detective Jake Peralta, a talented but carefree detective who's used to doing whatever he wants.",

    isBookmarked: true,

    isTrending: true,
  },

  {
    id: 14,

    title: 'Kantara',

    poster:
      'https://akm-img-a-in.tosshub.com/indiatoday/images/story/202210/Kantara_1_1200x768.jpeg?VersionId=cg8qm90nqeCXkGwEzydAKHyIh7KNrmCW',

    year: 2022,

    category: 'Movie',

    rating: 'UG',

    link: '8mrVmf239GU',

    description:
      "Kantara, which translates to mystical forest, tells the story of a local demigod (Bhoota) who trades forest land to the tribe's people with a king in 1870 in exchange for happiness. Many years later, when the king's son grows greedy and wants the land back, he dies due to the wrath of Bhoota",

    isBookmarked: true,

    isTrending: false,
  },
];
